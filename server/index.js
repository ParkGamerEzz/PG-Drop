...
import PGDropServer from "./server.js";
import PGDropWsServer from "./ws-server.js";
...
if (conf.signalingServer) {
    ...
    if (!isValidUrl || containsProtocol) {
        console.error("SIGNALING_SERVER must be a valid url without the protocol prefix.\n" +
            "Examples of valid values: `PGDrop.net`, `PGDrop.example.com:3000`, `example.com/PGDrop`");
        process.exit(1);
    }
...
// Start a new PGDrop instance when an uncaught exception occurs
if (conf.autoStart) {
    process.on(
        'uncaughtException',
        () => {
...
const pGDropServer = new PGDropServer(conf);

if (!conf.signalingServer) {
    // Start websocket server if SIGNALING_SERVER is not set
    new PGDropWsServer(pGDropServer.server, conf);
} else {
...
console.log('\nPGDrop is running on port', conf.port);
